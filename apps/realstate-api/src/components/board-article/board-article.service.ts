...
export class BoardArticleService {
    ...
    public async updateBoardArticle(memberId, input):Promise<BoardArticle>{
        const {_id, articleStatus} = input;
        const result = await this.boardArticleSchema
        .findOneAndUpdate({_id: _id, memberId, articleStatus:BoardArticleStatus.ACTIVE}, 
            input, {
            new: true
        }).exec();
    ...
    public async getBoardArticles(memberId: ObjectId, input: BoardArticlesInquiry):Promise<BoardArticles> {
        ...
        const result = await this.boardArticleSchema.aggregate([
            {$match: match},
            {$sort: sort},
            {
                $facet: {
    ...
    public async likeTargetBoardArticle(memberId: ObjectId, likeRefId: ObjectId):Promise<BoardArticle>{
		const target: BoardArticle = await this.boardArticleSchema.findOne({_id: likeRefId, articleStatus: BoardArticleStatus.ACTIVE}).exec();
    ...
    public async updateBoardArticleByAdmin(input: BoardArticleUpdate): Promise<BoardArticle> {
        ...
        const result = await this.boardArticleSchema
        .findOneAndUpdate( {_id: _id, articleStatus: BoardArticleStatus.ACTIVE}, input, {new: true}).exec();
        if(!result) throw new InternalServerErrorException(Message.UPDATE_FAILED);
    ...
    public async removeBoardArticleByAdmin(articleId: ObjectId):Promise<BoardArticle> {
        ...
        const result = await this.boardArticleSchema.findOneAndDelete(search).exec();
