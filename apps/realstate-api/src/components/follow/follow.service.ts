...
export class FollowService {
    constructor(@InjectModel("Follow") private readonly followSchema: Model<Follower | Following>,
    ...
    private async registerSubscription(followerId: ObjectId, followingId: ObjectId): Promise<Follower> {
        try {
            return await this.followSchema.create({
    ...
    public async unsubscribe(followerId: ObjectId, followingId: ObjectId):Promise<Follower> {
        ...
        const result = await this.followSchema.findOneAndDelete({
            followingId: followingId,
            followerId: followerId
        });
     ...
     public async getMemberFollowings(memberId: ObjectId, input: FollowInquiry):Promise<Followings> {
        ...
        const result = await this.followSchema.aggregate([
            {$match: match},
